...
class Model
{
    ...
    {
        $result = Database::query("SELECT max(id) as maxId FROM comments");
        $maxId = array();
        ...
        if (empty($out)) {
            $result2 = Database::query("SELECT * FROM comments ORDER BY id desc LIMIT 3");
        } else {
            $result2 = Database::query("SELECT * FROM comments ORDER BY id desc LIMIT $out, 3");
        }
        $comment = array();
    ...
    {
        $result = Database::query("SELECT count(id) as countFromDB FROM comments");
        $count = array();
